...
### 🤖 AI Tag Generation (NEW!)
- **Intelligent Behavior Analysis**: Automatically analyze track behavior patterns
- **Speed Classification**: Categorize tracks by speed (slow, moderate, fast, supersonic)
- **Maneuvering Detection**: Calculate G-forces and classify maneuvering intensity (2g-10g+)
- **Path Analysis**: Detect linear, curved, or serpentine flight patterns
- **Altitude Behavior**: Analyze climbing, descending, and cruise patterns
- **Engine Configuration**: Identify single, twin, or four-engine aircraft
- **Aircraft Role Inference**: Classify as commercial, military fighter, or general aviation
- **Automated Tagging**: Generate comprehensive tags for each track
...
2. **Install dependencies**
   ...
   - `pandas` - For data analysis and tag generation
...
5. **Generate AI Tags** (NEW!):
   - Click "🤖 Run AI Model generate Tags" to analyze tracks
   - The system will automatically:
     - Analyze track behavior (speed, maneuvering, path)
     - Calculate G-forces and flight characteristics
     - Generate intelligent tags for each track
     - Save tagged CSV with timestamp
   - View comprehensive statistics about generated tags
   - Tags include: speed categories, maneuvering levels (6g, 8g, 10g), path patterns, engine configuration, and more
   - See `AI_TAG_GENERATION_GUIDE.md` for detailed documentation
...
- **`track_tag_generator.py`**: AI model for intelligent tag generation (NEW!)
- **`requirements.txt`**: Python package dependencies
- **`AI_TAG_GENERATION_GUIDE.md`**: Comprehensive guide for tag generation feature (NEW!)
...
- **`airborne_tracks_tagged_<timestamp>.csv`**: CSV with AI-generated tags (NEW!)
...
Here are some example tags generated by the AI model:

### Commercial Airliner (Boeing 737)
```
moderate_speed, variable_speed, minimal_maneuvering, linear_path, 
straight_flight, cruise_altitude, twin_engine, commercial_airliner_profile
```

### Military Fighter (High Maneuvering)
```
very_fast_moving, high_maneuvering_6g_8g, curved_path, sharp_turns, 
medium_altitude, single_engine, military_fighter_profile, tactical_maneuvering
```

### General Aviation
```
slow_moving, minimal_maneuvering, linear_path, low_altitude, 
level_flight, single_engine, general_aviation_profile
```

See `AI_TAG_GENERATION_GUIDE.md` for complete documentation on all tag types and classification logic.
...
- [x] Track statistics and analytics with AI tag generation ✅
- [ ] Machine learning model training for custom tag generation
...
- **v1.1** (2025-11-12): AI Tag Generation Feature
  - AI-powered track behavior analysis
  - Intelligent tag generation (speed, maneuvering, path, altitude)
  - G-force calculation (6g, 8g, 10g detection)
  - Engine configuration identification
  - Aircraft role classification
  - Comprehensive tagging system with 13+ tag categories
